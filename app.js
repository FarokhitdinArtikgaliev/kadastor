(() => {
  ...
  function fileToDataUrl(file,maxSide=1280,quality=.78){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>{const img=new Image();img.onload=()=>{const scale=Math.min(1,maxSide/Math.max(img.width,img.height));const c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.width*scale));c.height=Math.max(1,Math.round(img.height*scale));c.getContext('2d').drawImage(img,0,0,c.width,c.height);resolve(c.toDataURL('image/jpeg',quality))};img.onerror=reject;img.src=r.result};r.onerror=reject;r.readAsDataURL(file)})}
  ...
  async function osrmRoute(points){
    if(points.length<2)return {km:0,min:0,legs:[]};
    try{const u=`https://router.project-osrm.org/route/v1/driving/${points.map(p=>`${p.lon},${p.lat}`).join(';')}?overview=false&steps=false`;const r=await fetch(u);if(!r.ok)throw 0;const j=await r.json();if(j.routes?.[0]){const route=j.routes[0];let cumulative=0;const legs=(route.legs||[]).map((leg,i)=>{const km=leg.distance/1000,min=leg.duration/60;cumulative+=km;return {from:points[i]?.name||'',to:points[i+1]?.name||'',km:Number(km.toFixed(1)),min:Math.round(min),cumulativeKm:Number(cumulative.toFixed(1))}});return {km:route.distance/1000,min:route.duration/60,road:true,legs};}}
    catch{}
    let km=0,min=0,legs=[];for(let i=1;i<points.length;i++){const legKm=hav(points[i-1],points[i])*1.18;const legMin=legKm/45*60;km+=legKm;min+=legMin;legs.push({from:points[i-1].name||'',to:points[i].name||'',km:Number(legKm.toFixed(1)),min:Math.round(legMin),cumulativeKm:Number(km.toFixed(1))})}return {km,min,road:false,legs};
  }
  ...
  async function startNavigation(){
    ...
    const trip={id:crypto.randomUUID?.()||String(Date.now()),startedAt:new Date().toISOString(),points:planner.points.slice(),origin:planner.origin.name,includeHomeReturn:planner.includeHomeReturn,km:Number(d.km.toFixed(1)),plannedMin:Math.round(d.min),legs:d.legs||[],samples:0,calculation:'Дорожный маршрут',navPhoto:null};
    activeTrip=trip;
  ...
  function finishTrip(){
    ...
    const already=new Set(visits().filter(v=>v.tripId===activeTrip.id&&v.status!=='Отменено').map(v=>v.branch));
    showModal(`<h2>Завершить поездку</h2><p class="muted">Расчётный пробег маршрута: <b>${num(activeTrip.km)} км</b>. Отметьте филиалы, которые фактически посетили.</p><div class="finish-visits">${names.map(n=>`<label class="check"><input type="checkbox" name="done" value="${esc(n)}" ${already.has(n)?'checked':''}> ${esc(n)}${already.has(n)?' · уже отмечен':''}</label>`).join('')}</div><label class="field">Фото из Яндекс Навигатора <span class="hint">Скриншот, где виден километраж маршрута</span><input id="navPhoto" type="file" accept="image/*"></label><div id="photoPreview" class="photo-preview"></div><div class="modal-actions"><button class="ghost" data-close>Отмена</button><button class="primary" id="finishTripSave">Сохранить поездку</button></div>`);
    let photoData=activeTrip.navPhoto||null;
    $('#navPhoto').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{photoData=await fileToDataUrl(file);$('#photoPreview').innerHTML=`<img src="${photoData}" alt="Скриншот Яндекс Навигатора">`}catch{alert('Не удалось загрузить фото.')}};
    $('#finishTripSave').onclick=()=>{
      const selected=[...document.querySelectorAll('input[name="done"]:checked')].map(x=>x.value);
      const now=new Date().toISOString();
      selected.forEach(n=>{if(!already.has(n)){const leg=(activeTrip.legs||[]).find(x=>x.to===n);saveVisit({branch:n,status:'Посещено',notes:'Посещение по завершённому маршруту',date:now,tripId:activeTrip.id,routeKm:leg?.cumulativeKm??null})}});
      const t={...activeTrip,finishedAt:now,date:now,visited:selected,navPhoto:photoData};
  ...
  function branchDetail(name){
    ...
    $('#visitNow').onclick=()=>{
      const now=new Date().toISOString();
      const leg=activeTrip?.legs?.find(x=>x.to===name);
      saveVisit({branch:name,status:'Посещено',notes:activeTrip?'Отмечено во время активного маршрута':'Отмечено вручную',date:now,tripId:activeTrip?.id||null,routeKm:leg?.cumulativeKm??null});closeModal();render()
    };
  }
  ...
  function dataUrlToFile(dataUrl,name='yandex-navigator.jpg'){
    const [head,b64]=dataUrl.split(',');const mime=(head.match(/data:(.*?);/)||[])[1]||'image/jpeg';const bin=atob(b64);const bytes=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)bytes[i]=bin.charCodeAt(i);return new File([bytes],name,{type:mime});
  }
  function reportText(t){
    const route=(t.origin?[t.origin]:[]).concat(t.points||[]).concat(t.includeHomeReturn?['Дом']:[]);
    const tripVisits=visits().filter(v=>v.tripId===t.id).sort((a,b)=>new Date(a.date)-new Date(b.date));
    const lines=[`Отчёт по поездке: ${dateFmt(t.date)}`,`Пробег: ${num(t.km)} км`,`Маршрут: ${route.join(' → ')}`,`Посещено филиалов: ${(t.visited||[]).length}`];
    (t.visited||[]).forEach(n=>{const v=tripVisits.find(x=>x.branch===n);lines.push(`• ${n} — ${v?dateFmt(v.date):dateFmt(t.finishedAt||t.date)}${v?.routeKm!=null?` — ${num(v.routeKm)} км`:''}`)});
    lines.push(`Компенсация: ${money(Number(t.km||0)*settings().rate)}`);return lines.join('\n');
  }
  async function shareTripReport(t){
    const text=reportText(t);
    try{if(navigator.share){const data={title:'Отчёт по поездке',text};if(t.navPhoto&&navigator.canShare){const f=dataUrlToFile(t.navPhoto,`yandex-${t.id}.jpg`);if(navigator.canShare({files:[f]})){data.files=[f]}}await navigator.share(data);return}}catch(e){}
    try{await navigator.clipboard.writeText(text);alert('Текст отчёта скопирован. Фото можно открыть в отчёте.')}catch{alert(text)}
  }
  async function attachTripPhoto(id,file){
    try{const data=await fileToDataUrl(file);const arr=trips();const i=arr.findIndex(t=>t.id===id);if(i<0)return;arr[i]={...arr[i],navPhoto:data};save('trips',arr);renderReports()}catch{alert('Не удалось загрузить фото.')} 
  }
  function tripReportCard(t){
    const comp=Number(t.km||0)*settings().rate;
    const route=(t.origin?[t.origin]:[]).concat(t.points||[]).concat(t.includeHomeReturn?['Дом']:[]);
    const tripVisits=visits().filter(v=>v.tripId===t.id).sort((a,b)=>new Date(a.date)-new Date(b.date));
    const visitedHtml=(t.visited||[]).map(n=>{const v=tripVisits.find(x=>x.branch===n);return `<div class="visit-line"><b>${esc(n)}</b><span>${v?dateFmt(v.date):dateFmt(t.finishedAt||t.date)}${v?.routeKm!=null?` · ${num(v.routeKm)} км`:''}</span></div>`}).join('');
    return `<article class="trip-report"><div class="trip-report-head"><div><div class="eyebrow">Поездка</div><h3>${dateFmt(t.date)}</h3></div><b class="trip-km">${num(t.km)} км</b></div><div class="trip-route">${esc(route.join(' → '))}</div><div class="trip-meta"><span>Посещено: <b>${(t.visited||[]).length}</b></span><span>Компенсация: <b>${money(comp)}</b></span></div>${visitedHtml?`<div class="visit-lines">${visitedHtml}</div>`:''}${t.navPhoto?`<div class="nav-photo-wrap"><div class="section-head compact"><h4>Фото Яндекс Навигатора</h4><small>Подтверждение километража</small></div><img class="nav-photo" src="${t.navPhoto}" alt="Скриншот Яндекс Навигатора с километражом"><div class="report-actions"><button class="ghost small" data-photo="${esc(t.id)}">Открыть фото</button><button class="ghost small" data-share="${esc(t.id)}">Поделиться</button></div></div>`:`<div class="photo-missing">Фото Яндекс Навигатора не прикреплено<label class="photo-upload">+ Добавить скриншот<input type="file" accept="image/*" data-upload-photo="${esc(t.id)}"></label></div>`}</article>`;
  }
  function renderReports(){const vs=visits().sort((a,b)=>new Date(b.date)-new Date(a.date)),ts=trips();const rows=vs.map(v=>`<article class="report-row"><div><b>${esc(v.branch)}</b><span>${esc(v.status)}${v.routeKm!=null?` · ${num(v.routeKm)} км по маршруту`:''}</span><small>${dateFmt(v.date)}</small>${v.notes?`<p>${esc(v.notes)}</p>`:''}</div><button class="ghost small" data-editvisit="${esc(v.id||v.date)}">Изменить</button></article>`).join('');$('#screen').innerHTML=`<section class="page"><div class="page-title"><div><div class="eyebrow">Журнал работы</div><h1>Отчёты</h1><p>Километраж, время посещения филиалов и подтверждающие фото.</p></div><button class="primary small" id="addVisit">+ Посещение</button></div><div class="periods"><button class="period active" data-period="all">Все</button><button class="period" data-period="today">Сегодня</button><button class="period" data-period="week">Неделя</button><button class="period" data-period="month">Месяц</button></div><div id="reportList" class="report-list">${ts.length?ts.slice(0,20).map(tripReportCard).join(''):'<div class="empty">Поездок пока нет.</div>'}</div><div class="card"><div class="section-head"><h2>Посещения</h2><span>${vs.length}</span></div>${rows||'<div class="empty">Посещений пока нет.</div>'}</div></section>`;
    $('#addVisit').onclick=()=>openVisitForm();$$('[data-editvisit]').forEach(b=>b.onclick=()=>openVisitForm(vs.find(v=>(v.id||v.date)===b.dataset.editvisit)));$$('[data-photo]').forEach(b=>b.onclick=()=>{const t=trips().find(x=>x.id===b.dataset.photo);if(t?.navPhoto)showModal(`<h2>Яндекс Навигатор</h2><img class="nav-photo large" src="${t.navPhoto}" alt="Скриншот Яндекс Навигатора"><div class="modal-actions"><button class="ghost" data-close>Закрыть</button></div>`) });$$('[data-share]').forEach(b=>b.onclick=()=>{const t=trips().find(x=>x.id===b.dataset.share);if(t)shareTripReport(t)});$$('[data-upload-photo]').forEach(i=>i.onchange=e=>{const f=e.target.files?.[0];if(f)attachTripPhoto(i.dataset.uploadPhoto,f)});
    $$('.period').forEach(b=>b.onclick=()=>filterReports(b.dataset.period));
  }
  function filterReports(period){const now=new Date();let ts=trips();if(period==='today')ts=ts.filter(t=>new Date(t.date).toDateString()===now.toDateString());if(period==='week'){const d=new Date(now);d.setDate(now.getDate()-6);ts=ts.filter(t=>new Date(t.date)>=d)}if(period==='month')ts=ts.filter(t=>new Date(t.date).getMonth()===now.getMonth()&&new Date(t.date).getFullYear()===now.getFullYear());$('#reportList').innerHTML=ts.map(tripReportCard).join('')||'<div class="empty">Нет поездок за выбранный период.</div>';$$('[data-photo]').forEach(b=>b.onclick=()=>{const t=trips().find(x=>x.id===b.dataset.photo);if(t?.navPhoto)showModal(`<h2>Яндекс Навигатор</h2><img class="nav-photo large" src="${t.navPhoto}" alt="Скриншот Яндекс Навигатора"><div class="modal-actions"><button class="ghost" data-close>Закрыть</button></div>`) });$$('[data-share]').forEach(b=>b.onclick=()=>{const t=trips().find(x=>x.id===b.dataset.share);if(t)shareTripReport(t)});$$('[data-upload-photo]').forEach(i=>i.onchange=e=>{const f=e.target.files?.[0];if(f)attachTripPhoto(i.dataset.uploadPhoto,f)});}
