(() => {
  ...
  function finishTrip(){
    ...
    $('#finishTripSave').onclick=()=>{const now=new Date().toISOString(),old=visits();const result=[];names.forEach(n=>{const q=`[data-visit-status="${CSS.escape(n)}"]`,qn=`[data-visit-note="${CSS.escape(n)}"]`,status=$(q)?.value||'Посещено',notes=$(qn)?.value?.trim()||'';result.push({branch:n,status,notes});old.unshift({id:crypto.randomUUID?.()||String(Date.now()+Math.random()),branch:n,status,notes,date:now,tripId:activeTrip.id})});save('visits',old.slice(0,2000));const t={...activeTrip,finishedAt:now,date:now,visited:names.slice(),visitResults:result,routeNames:activeTrip.routeNames||[activeTrip.origin,...names,...(activeTrip.includeHomeReturn?['Дом']:[])]};const a=trips();a.unshift(t);save('trips',a);save('activeTrip',null);activeTrip=null;lastGps=null;closeModal();render()};
  }
