...
(function(){
  ...
  function openBranchCard(id){
    ...
    const html=r.stops.map((s,i)=>{const p=stopCoords[s], ph=branchPhones[s], sp=staffPhones[s];
      ...
      <div class="contact-line"><span>👨‍🔧 Ответственный</span><b>${sp?esc(sp):'не указан'}</b></div><div class="contact-line"><span>👔 Филиал бошлиғи</span><b>${ph?esc(ph):'не указан / вакант'}</b></div></div>
  ...
  function reportItems(period){const now=new Date();let start=new Date(now);if(period==='day')start.setHours(0,0,0,0);if(period==='week'){const day=(now.getDay()+6)%7;start.setDate(now.getDate()-day);start.setHours(0,0,0,0);}if(period==='month')start=new Date(now.getFullYear(),now.getMonth(),1);const visits=getVisits().filter(v=>new Date(v.date)>=start);const trips=getTripHistory().filter(t=>new Date(t.date)>=start);const km=trips.reduce((a,t)=>a+Number(t.km||0),0),c=costs(km);return {period,start,visits,trips,km,c};}
